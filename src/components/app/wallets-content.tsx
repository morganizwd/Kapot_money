  ...
  return (
    <PageContainer className="grid gap-7">
      ...
      <section>
        ...
        {wallets.length === 0 ? <div className="mt-3 rounded-[var(--radius-card)] bg-muted px-5 py-7 text-center"><WalletCards aria-hidden className="mx-auto size-7 text-primary" /><p className="mt-3 font-semibold">Нет кошельков</p><p className="mt-1 text-sm text-muted-foreground">Создайте первый, чтобы начать учитывать баланс.</p><Button className="mt-5" onClick={() => setSheet("wallet")}>Создать кошелёк</Button></div> : <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{wallets.map((wallet) => <article key={wallet.id} className="rounded-[var(--radius-card)] border border-border bg-panel p-5 shadow-[var(--shadow-panel)]"><span className="grid size-11 place-items-center rounded-xl bg-accent text-accent-foreground"><WalletCards aria-hidden className="size-5" /></span><p className="mt-4 truncate text-base font-semibold">{wallet.name}</p><p className="mt-1 text-sm text-muted-foreground">{walletTypeLabels[wallet.type] ?? wallet.type}</p><MoneyAmount amountMinor={wallet.current_balance_minor} currencyCode={wallet.currency_code} className="mt-5 text-2xl font-bold" /><button type="button" onClick={() => { setSelectedWallet(wallet); setSheet("balance"); }} className="mt-4 inline-flex min-h-10 items-center gap-2 text-sm font-semibold text-primary transition hover:text-primary/80"><PencilLine aria-hidden className="size-4" />Изменить баланс</button>{!wallet.include_in_net_worth ? <p className="mt-3 text-xs font-semibold text-muted-foreground">Не входит в общий баланс</p> : null}</article>)}</div>}
      </section>
